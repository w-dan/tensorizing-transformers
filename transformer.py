...
class Encoder(nn.Module):
    def __init__(
        ...
        self.embed_size = embed_size                    # positional embedding
        self.device = device        # Embedding?
...
class Transformer(nn.Module):
    ...
    def make_trg_mask(self, trg):
        N, trg_len = trg.shape
        trg_mask = torch.tril(torch.ones(trg_len, trg_len)).expand(     # triangular matrix for mask
            N, 1, trg_len, trg_len
        )
        return trg_mask
